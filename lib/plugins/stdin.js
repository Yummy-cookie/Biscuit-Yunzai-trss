...
Bot.adapter.push(new class stdinAdapter {
      ...
      switch (i.type) {
        case "text":
          if (i.data.text.match("\n"))
            i.data.text = `\n${i.data.text}`
          logger.info(`[${this.id}] 发送文本：${i.data.text}`)
          break
        case "image":
          logger.info(`[${this.id}] 发送图片：${file.url}\n文件已保存到：${file.path}`)
          fs.writeFileSync(file.path, file.buffer)
          break
        case "record":
          logger.info(`[${this.id}] 发送音频：${file.url}\n文件已保存到：${file.path}`)
          fs.writeFileSync(file.path, file.buffer)
          break
        case "video":
          logger.info(`[${this.id}] 发送视频：${file.url}\n文件已保存到：${file.path}`)
          fs.writeFileSync(file.path, file.buffer)
          break
        case "reply":
          break
  ...
  message(msg) {
    ...
    logger.info(`[${data.self_id}] 系统消息：[${data.sender.nickname}(${data.user_id})] ${data.raw_message}`)
  ...
  load() {
    ...
    logger.mark(`[${this.id}] ${this.name}(${this.id}) 已连接`)
